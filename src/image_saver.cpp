...
void ImageSaver::save(const std::unique_ptr<Vector3D<float>[]> &image) {
    ...
    for (size_t y = 0; y < _options->height; y++) {
        for (size_t x = 0; x < _options->width; x++) {
            char r = (char)(255 * std::min(1.0f, image[y * _options->height + x].X()));
            char g = (char)(255 * std::min(1.0f, image[y * _options->height + x].Y()));
            char b = (char)(255 * std::min(1.0f, image[y * _options->height + x].Z()));
            _outstream << r << g << b;
        }
    }
